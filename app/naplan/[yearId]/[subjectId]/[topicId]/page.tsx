...
type QuestionData = typeof questionDataNap;
type YearKey = keyof QuestionData;
type SubjectKey = keyof QuestionData[YearKey];
type TopicKey = keyof QuestionData[YearKey][SubjectKey];
...
// const ExamPageNaplan = ({ params }: ExamProps) => {
//   const { yearId, subjectId, topicId } = params;

export default function ExamPageNaplan({
  params,
}: {
  params: Promise<{ yearId: string; subjectId: string; topicId: string }>;
}) {
  const { yearId, subjectId, topicId } = React.use(params);
  // Fix year format (year-2 → year2)
  const yearKey = yearId.replace("-", "") as YearKey;
  const subjectKey = subjectId as SubjectKey;
  const topicKey = topicId as TopicKey;

  const topicData = questionDataNap[yearKey]?.[subjectKey]?.[topicKey];
