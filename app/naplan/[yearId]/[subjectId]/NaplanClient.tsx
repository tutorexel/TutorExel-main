...
// export default function NaplanDataPage({
//   params,
// }: {
...
type NaplanPageProps = {
  yearId: string;
  subjectId: string;
};
export default function NaplanDataPage({ yearId, subjectId }: NaplanPageProps) {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [yearId, subjectId]);
  ...
  const subjectContent =
    naplanData[yearId as keyof typeof naplanData]?.[
            ...
            <p className="mb-0">
              <i>
                {subjectsByYear.map((y, index) => {
                  ...
                  return (
                    <span key={y.year}>
                      <Link href={`/naplan/year-${yearNumber}/${subjectId}`}>
          ...
          <Row>
            <p className="mb-0">
              <span className={subjectId === "maths" ? "highlight p-3" : "p-3"}>
                <Link href={`/naplan/${yearId}/maths`}>Maths</Link>
              </span>
              <span
                className={subjectId === "english" ? "highlight p-3" : "p-3"}
              >
                <Link href={`/naplan/${yearId}/english`}>English</Link>
              </span>
            </p>
          </Row>
        </Container>
      ...
      <section className="pt-5 bg-white">
        <Container>
          <Row className="">
            ...
            <h4>
              <i>
                <b>Click on any Topic below to practise exam-style questions</b>
              </i>
            </h4>
          </Row>
        </Container>
                    ...
                    {subjectContent.term.topics.map((item, index) => (
                      <Row key={index} className="term-row gx-0">
                        ...
                        <Col xs={4} className="td name">
                          <Link
                            href={`/naplan/${yearId}/${subjectId}/topic${index + 1}`}
                          >
                            {item.name}
                          </Link>
                        </Col>
